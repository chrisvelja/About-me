...
const b64=a=>{let s='';new Uint8Array(a).forEach(v=>s+=String.fromCharCode(v));return btoa(s).replace(/\\+/g,'-').replace(/\\//g,'_').replace(/=+$/,'')};
async function login(){
  try{
    ...
    const params=new URLSearchParams({client_id:CLIENT,response_type:'code',redirect_uri:REDIRECT,scope:SCOPE,code_challenge_method:'S256',code_challenge:challenge});
    window.location.assign('https://accounts.spotify.com/authorize?'+params.toString());
  }catch(err){
    console.error('Spotify login failed:',err);
    const box=document.getElementById('spotify-connect');
    if(box) box.querySelector('p').textContent='Spotify could not be opened. Please check the Spotify app Redirect URI settings.';
  }
}
...
async function auth(){
  ...
  if(error){console.warn('Spotify authorization error:',error);history.replaceState(null,'',window.location.pathname+window.location.hash);return;}
  if(!code)return;
